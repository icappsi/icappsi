...
async function guardarUsuario() {
  ...
  if (!cedula) { alert('⚠️ La cédula es obligatoria'); return; }
  if (!nombre) { alert('⚠️ El nombre es obligatorio'); return; }
  if (!apellido) { alert('⚠️ El apellido es obligatorio'); return; }
  if (!jerarquia) { alert('⚠️ Debes seleccionar una jerarquía'); return; }
  if (!expediente) { alert('⚠️ El número de expediente es obligatorio'); return; }
  if (!causaSancion) { alert('⚠️ La causa de sanción es obligatoria'); return; }
  if (!/^\d{7,8}$/.test(cedula)) { alert('La cédula debe contener entre 7 y 8 dígitos numéricos'); return; }
  if (nivel === 'administrador' && !usuarioLogueado.es_super_admin) { alert('⚠️ Solo el Super Administrador puede crear administradores'); return; }
  if (nivel === 'administrador' && !usuarioEditandoId && !password) { alert('🔐 Los administradores DEBEN tener una contraseña'); return; }
  if (password && password.length < 8) { alert('🔐 La contraseña debe tener al menos 8 caracteres'); return; }
  if (esSuperAdmin && !usuarioLogueado.es_super_admin) { alert('⚠️ Solo el Super Administrador puede crear otros Super Administradores'); return; }
